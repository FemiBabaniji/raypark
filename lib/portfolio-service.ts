...
export async function createPortfolioOnce(params: {
  ...
  const { data: page, error: pageErr } = await supabase
    ...
    .single()
  if (!page && pageErr) {
    console.warn("[seed] page create failed:", pageErr?.message)
    return data
  }

  const { data: widgetTypes, error: widgetTypesError } = await supabase
    .from("widget_types")
    .select("id, key")
    .in("key", ["profile", "education", "startup", "description", "projects", "services"])

  if (widgetTypesError) {
    console.warn("[seed] widget types fetch failed:", widgetTypesError?.message)
  }
  ...
  // Left column widgets
  if (widgetTypeMap.has("profile")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("profile"),
      props: {
        name: params.name,
        title: "Your Title",
  ...
  if (widgetTypeMap.has("education")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("education"),
      props: {
  ...
  // Right column widgets - Startup widget first!
  if (widgetTypeMap.has("startup")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("startup"),
      props: {
  ...
  if (widgetTypeMap.has("description")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("description"),
      props: {
        title: "About Me",
        content: "Write a brief description about yourself and what you do...",
      },
      enabled: true,
    })
  }

  if (widgetTypeMap.has("projects")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("projects"),
      props: {
        title: "My Projects",
        projects: [
          {
  ...
  if (widgetTypeMap.has("services")) {
    defaultWidgets.push({
      page_id: page.id,
      widget_type_id: widgetTypeMap.get("services"),
      props: {
        title: "What I Offer",
        description: "Describe your services or offerings...",
      },
      enabled: true,
    })
  }
  ...
  if (defaultWidgets.length > 0) {
    const { error: widgetsError } = await supabase.from("widget_instances").insert(defaultWidgets)

    if (widgetsError) {
      console.warn("[seed] widgets create failed:", widgetsError?.message)
    }
  }

  const { data: createdWidgets, error: widgetsQueryError } = await supabase
    .from("widget_instances")
    .select("id, widget_type_id")
    .eq("page_id", page.id)
    .order("created_at", { ascending: true })

  if (widgetsQueryError) {
    console.warn("[seed] widgets query failed:", widgetsQueryError?.message)
  }

  const leftWidgetIds: string[] = []
  const rightWidgetIds: string[] = []

  createdWidgets?.forEach((widget) => {
    const widgetKey = Array.from(widgetTypeMap.entries()).find(([_, id]) => id === widget.widget_type_id)?.[0]

    // Left column: profile, education
    if (widgetKey === "profile" || widgetKey === "education") {
      leftWidgetIds.push(widget.id)
    }
    // Right column: startup (first!), description, projects, services
    else if (
      widgetKey === "startup" ||
