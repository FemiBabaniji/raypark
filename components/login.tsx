...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log("[v0] 🔐 Login: Starting Google OAuth flow")
      console.log(
        "[v0] 🔐 Login: Redirect URL will be:",
      ...
      if (error) {
        console.error("[v0] ❌ Login: OAuth error:", error.message)
        setError(error.message)
      } else {
        console.log("[v0] ✅ Login: OAuth redirect initiated")
      }
    } catch (err) {
